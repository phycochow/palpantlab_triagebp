...
steps of the generation of RTS values.
"""
...
def normalise_breadth(input_file):
    ...
    with open(input_file, 'r') as in_f:
        ...
        data['normalised_breadth'] = (data['temp_breadth'] - min_value) / (max_value - min_value)
        ...
        with open(output_file, 'w') as out_f:
            data.to_csv(out_f, sep='\t', header=False, index=False,
                        columns=['chromosome', 'start', 'end', 'normalised_breadth'], float_format='%.4e')
